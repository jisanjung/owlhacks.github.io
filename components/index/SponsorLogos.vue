<template>
  <div class="py-2 text-center">
    <h2 class="text-red" style="font-size: 2.3rem">Sponsors</h2>
    <div class="logos-ctn">
      <h3 style="color: #99d1ff">Title</h3>
      <div class="col-xl-12">
        <img src="/sponsors/title/vanguard-logo.png" width="250" height="60" />
        <img src="/sponsors/title/cigna-logo.png" width="250" height="80" />
        <img src="/sponsors/title/elsevier-logo.png" width="140" height="140" />
        <img
          src="/sponsors/title/lockheedmartin-logo.png"
          width="250"
          height="120"
        />
        <img
          src="/sponsors/title/lincolnfinancial-logo.png"
          width="280"
          height="90"
        />
      </div>
    </div>
    <div class="logos-ctn">
      <h3 style="margin-top: 15px; color: #FFD700">Gold</h3>
      <div class="col-xl-12">
        <img
          src="/sponsors/gold/brooksource-logo.png"
          width="270"
          height="110"
        />
        <img src="/sponsors/gold/emoney-logo.png" width="250" height="70" />
      </div>
    </div>
    <div class="logos-ctn">
      <h3 style="color: #aeb3b7">Silver</h3>
      <div class="col-xl-12">
        <img src="/sponsors/silver/google-logo.png" width="240" height="80" />
        <img
          class="mx-0"
          src="/sponsors/silver/almac-logo.png"
          width="240"
          height="100"
        />
        <img src="/sponsors/silver/guru-logo.png" width="220" height="48" />
        <img src="/sponsors/silver/linode-logo.png" width="220" height="80" />
      </div>
    </div>
  </div>
</template>

<style scoped>
.logos-ctn {
  justify-content: center;
}

.logos-ctn img {
  margin: 5px 10px;
}
</style>
